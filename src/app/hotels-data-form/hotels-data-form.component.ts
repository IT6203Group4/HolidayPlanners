...
export class HotelsDataFormComponent implements OnInit {
  ...
  onSubmit(){
    console.log("You submitted: " + this.Destination + " " + this.Checkin+ " "+this.Checkout+ " "+this.class+" "+this.PropertyType + " "+this.Guests + " ");
    if(this.mode =='add')
    this._myService.addHotelsdata(this.Destination,this.Checkin,this.Checkout,this.class,this.PropertyType,this.Guests);
 if(this.mode =='edit')
 this._myService.updateHotel(this.id,this.Destination,this.Checkin,this.Checkout,this.class,this.PropertyType,this.Guests);
